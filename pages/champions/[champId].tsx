...
import { BsFillShieldFill, BsMagic } from "react-icons/bs";
...
export function generateStars(difficulty: number) {
  const totalStars = 10;
...
export default function Champion({ data }: any) {
  ...
  const difficultyStars = generateStars(data.info.difficulty);
  const attackStars = generateStars(data.info.attack);
  const defenseStars = generateStars(data.info.defense);
  const magicStars = generateStars(data.info.magic);
  ...
  return (
    <>
      ...
      <SimpleGrid>
        ...
        <MediaQuery
          largerThan={1500}
          styles={{ width: "1500px", height: "620px" }}
        >
          <StyledCard
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
